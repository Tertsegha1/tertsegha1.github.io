...
{
  key:'week7', num:7, title:'Two Factors Are Better Than One',
  scenarioTag:'Real world: what if the password ALONE isn\'t the only thing standing between an attacker and an account?',
  scenario:`Beginner Week 6 checked a password by hashing and comparing. But a password is a single point of
    failure — if it leaks (reused elsewhere, guessed, phished), that's the WHOLE defense gone. This week adds a
    genuinely independent second check: a one-time code, on top of the password, so a stolen password alone is no
    longer enough to get in.`,
  objectives:[
    'Check a password using the hash-and-compare pattern from Beginner Week 6',
    'Check a second, independent factor (a one-time code)',
    'Require BOTH factors together before allowing login',
    'Demonstrate that a correct password with a wrong code still fails — and vice versa'
  ],
  conceptHtml:`
    <p>The password check, exactly as in Beginner Week 6:</p>
    <pre class="code-block">import hashlib
...
def check_password(input_pw, stored_hash):
    return hash_password(input_pw) == stored_hash</pre>
    <p>A second, INDEPENDENT factor — a one-time code (in a real system, sent to a phone or generated by an app,
    never derivable from the password itself):</p>
    <pre class="code-block">def check_second_factor(input_code, expected_code):
    return input_code == expected_code</pre>
    <p>Requiring BOTH together is what makes two-factor authentication genuinely stronger — not "either one,"
    but an AND:</p>
    <pre class="code-block">def login_with_mfa(input_pw, stored_hash, input_code, expected_code):
    return check_password(input_pw, stored_hash) and check_second_factor(input_code, expected_code)

stored_hash = "9b0eb22aef89516d6fb4b31ccf008a68abe0d10a3fc606316389613eccf96854"   # hash of "letmein123"
expected_code = "482913"

print(login_with_mfa("letmein123", stored_hash, "482913", expected_code))   # True  — both correct
print(login_with_mfa("letmein123", stored_hash, "111111", expected_code))   # False — right password, wrong code
print(login_with_mfa("wrongpass",  stored_hash, "482913", expected_code))   # False — right code, wrong password</pre>
    <p>Notice: an attacker who somehow stole the password ALONE still can't log in — they'd also need the second
    factor, which they don't have. That's the entire point of requiring two INDEPENDENT things, not two chances
    at the same thing.</p>`,
  sandboxStarter:`import hashlib
...
stored_hash = "9b0eb22aef89516d6fb4b31ccf008a68abe0d10a3fc606316389613eccf96854"
print(check_password("letmein123", stored_hash))
print(check_password("wrongpass", stored_hash))
`,
  sandboxStarter2:`def check_second_factor(input_code, expected_code):
    return input_code == expected_code

print(check_second_factor("482913", "482913"))
print(check_second_factor("111111", "482913"))
`,
  exercises:[
    {
      title:'Check the password',
      desc:`Write hash_password(pw) and check_password(input_pw, stored_hash) exactly as in Beginner Week 6.
        Using stored_hash = "9b0eb22aef89516d6fb4b31ccf008a68abe0d10a3fc606316389613eccf96854" (the hash of
        "letmein123"), assert that check_password("letmein123", stored_hash) == True and
        check_password("wrongpass", stored_hash) == False.`,
      starter:`import hashlib

stored_hash = "9b0eb22aef89516d6fb4b31ccf008a68abe0d10a3fc606316389613eccf96854"
# Write hash_password(pw) and check_password(input_pw, stored_hash) below
`,
      tests:[
        {type:'assert', expr:'check_password("letmein123", stored_hash) == True', label:'The correct password is correctly accepted'},
        {type:'assert', expr:'check_password("wrongpass", stored_hash) == False', label:'An incorrect password is correctly rejected'}
      ]
    },
    {
      title:'Check the second factor',
      desc:`Write check_second_factor(input_code, expected_code) that returns whether the two codes match.
        Assert that check_second_factor("482913", "482913") == True and
        check_second_factor("111111", "482913") == False.`,
      starter:`# Write check_second_factor(input_code, expected_code) below
`,
      tests:[
        {type:'assert', expr:'check_second_factor("482913", "482913") == True', label:'The correct code is correctly accepted'},
        {type:'assert', expr:'check_second_factor("111111", "482913") == False', label:'An incorrect code is correctly rejected'}
      ]
    },
    {
      title:'Require both factors together',
      desc:`Using check_password and check_second_factor (given), write
        login_with_mfa(input_pw, stored_hash, input_code, expected_code) returning True only if BOTH checks pass.
        Assert that login_with_mfa("letmein123", stored_hash, "482913", "482913") == True.`,
      starter:`import hashlib
...
def check_second_factor(input_code, expected_code):
    return input_code == expected_code

stored_hash = "9b0eb22aef89516d6fb4b31ccf008a68abe0d10a3fc606316389613eccf96854"
# Write login_with_mfa(input_pw, stored_hash, input_code, expected_code) below
`,
      tests:[{type:'assert', expr:'login_with_mfa("letmein123", stored_hash, "482913", "482913") == True', label:'Both factors correct correctly succeeds'}]
    },
    {
      title:'A stolen password alone is NOT enough',
      desc:`Using login_with_mfa (given), assert that
        login_with_mfa("letmein123", stored_hash, "111111", "482913") == False — the RIGHT password with the
        WRONG code still fails to log in.`,
      starter:`import hashlib
...
def login_with_mfa(input_pw, stored_hash, input_code, expected_code):
    return check_password(input_pw, stored_hash) and check_second_factor(input_code, expected_code)

stored_hash = "9b0eb22aef89516d6fb4b31ccf008a68abe0d10a3fc606316389613eccf96854"
`,
      tests:[{type:'assert', expr:'login_with_mfa("letmein123", stored_hash, "111111", "482913") == False', label:'A correct password with the wrong code correctly fails'}]
    },
    {
      title:'A guessed code alone is NOT enough either',
      desc:`Using login_with_mfa (given), assert that
        login_with_mfa("wrongpass", stored_hash, "482913", "482913") == False — the RIGHT code with the WRONG
        password still fails, since BOTH are required.`,
      starter:`import hashlib
...
stored_hash = "9b0eb22aef89516d6fb4b31ccf008a68abe0d10a3fc606316389613eccf96854"
`,
      tests:[{type:'assert', expr:'login_with_mfa("wrongpass", stored_hash, "482913", "482913") == False', label:'A correct code with the wrong password correctly fails'}]
    },
    {
      title:'Compare MFA against password-only across several attempts',
      desc:`Using login_with_mfa and check_password (given), stored_hash (given), expected_code = "482913", and
        attempts = [("letmein123","482913"), ("letmein123","000000"), ("wrongpass","482913"),
        ("letmein123","482913"), ("wrongpass","000000")], calculate mfa_success_count (attempts that succeed with
        BOTH factors) and password_only_success_count (attempts that would have succeeded with JUST the password
        check). Assert that mfa_success_count == 2 and password_only_success_count == 3 — MFA correctly blocks 1
        extra login that password-only checking would have wrongly allowed through.`,
      starter:`import hashlib
...
stored_hash = "9b0eb22aef89516d6fb4b31ccf008a68abe0d10a3fc606316389613eccf96854"
expected_code = "482913"
attempts = [("letmein123","482913"), ("letmein123","000000"), ("wrongpass","482913"), ("letmein123","482913"), ("wrongpass","000000")]
# Calculate mfa_success_count and password_only_success_count below
`,
      tests:[
        {type:'assert', expr:'mfa_success_count == 2', label:'mfa_success_count correctly equals 2'},
        {type:'assert', expr:'password_only_success_count == 3', label:'password_only_success_count correctly equals 3'}
      ]
    }
  ],
  quiz:[
    {
      q:'Why does two-factor authentication require the password AND the code, rather than either one?',
      options:['Requiring both is slower for no benefit','If either one alone was enough, an attacker who stole just ONE of the two factors could still get in — requiring BOTH means stealing only one factor achieves nothing','Passwords don\'t work without a second factor','The AND is a coding mistake, it should be OR'],
      correct:1,
      explain:'The whole security benefit comes from requiring genuinely independent factors TOGETHER — an attacker needs to compromise both, not just one.'
    },
    {
      q:'In this week\'s example, why is the "second factor" a genuinely independent check rather than another password?',
      options:['It isn\'t independent — it\'s just a second password','A second factor (like a one-time code sent elsewhere) can\'t be derived from or guessed alongside the password — stealing the password gives an attacker zero information about the code','Codes and passwords are stored the same way','It doesn\'t matter what the second factor is'],
      correct:1,
      explain:'True independence is the point — if the second factor could be inferred from the password, it wouldn\'t add any real protection.'
    },
    {
      q:'In the batch comparison exercise, why was mfa_success_count LOWER than password_only_success_count?',
      options:['MFA has a bug that rejects valid logins','One attempt had the correct password but the wrong code — password-only checking would have wrongly let it through, but MFA correctly caught it','The two counts should always be equal','password_only_success_count counted incorrectly'],
      correct:1,
      explain:'That specific attempt is exactly the scenario MFA is designed to catch — a correct password isn\'t enough on its own anymore.'
    },
    {
      q:'What real-world event does this week\'s "stolen password alone is not enough" lesson protect against?',
      options:['A user forgetting their password','A password being leaked, reused elsewhere, or guessed — MFA means that leak alone doesn\'t hand over the account','A slow internet connection','Two people sharing the same account on purpose'],
      correct:1,
      explain:'Passwords leak surprisingly often (data breaches, reuse across sites, phishing) — MFA is specifically designed to survive that exact failure.'
    }
  ],
  sandboxStarter3:`import hashlib
...
for pw, code in [("letmein123","482913"), ("letmein123","111111"), ("wrongpass","482913")]:
    print(pw, code, "->", login_with_mfa(pw, stored_hash, code, expected_code))
`,
  stretchChallenge:{
    title:'Find every attempt that MFA blocks but password-only would allow',
    desc:`Using login_with_mfa and check_password (given), stored_hash and expected_code (given), and
      stretch_attempts = [("letmein123","482913"), ("letmein123","999999"), ("wrongpass","482913"),
      ("wrongpass","111111"), ("letmein123","482913")], calculate stretch_blocked_by_mfa, a list of the
      (password, code) tuples where check_password succeeds but login_with_mfa does NOT (i.e. MFA specifically
      caught a right-password/wrong-code case). Assert that
      stretch_blocked_by_mfa == [("letmein123", "999999")].`,
    starter:`import hashlib
...
stored_hash = "9b0eb22aef89516d6fb4b31ccf008a68abe0d10a3fc606316389613eccf96854"
expected_code = "482913"
stretch_attempts = [("letmein123","482913"), ("letmein123","999999"), ("wrongpass","482913"), ("wrongpass","111111"), ("letmein123","482913")]
# Calculate stretch_blocked_by_mfa below
`,
    tests:[
